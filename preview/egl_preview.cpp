...
namespace {
  //{{{
  GLint compile_shader (GLenum target, const char* source) {
  ...
  //}}}
  //{{{
  void get_colour_space_info (std::optional<libcamera::ColorSpace> const& cs, EGLint &encoding, EGLint& range) {
  ...
  //}}}
  //{{{
  void no_border (Display* display, Window window) {
...
EglPreview::EglPreview (Options const* options) 
   : Preview(options), last_fd_(-1), first_time_(true) {
...
void EglPreview::makeWindow (char const* name) {
  ...
  window_ = XCreateWindow (display_, root, x_, y_, width_, height_, 0, visinfo->depth,
...
Preview* make_egl_preview (Options const* options) { return new EglPreview (options); }
