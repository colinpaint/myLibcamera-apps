...
public:
  ...
  // Display the buffer. You get given the fd back in the BufferDoneCallback once its available for re-use.
  virtual void Show (int fd, libcamera::Span<uint8_t> span, StreamInfo const &info) override;

  // Reset the preview window, clearing the current buffers and being ready to show new ones.
...
//}}}
DrmPreview::~DrmPreview() { close(drmfd_); }
...
void DrmPreview::makeBuffer (int fd, size_t size, StreamInfo const &info, Buffer &buffer) {
  ...
  uint32_t offsets[4] = { 0, info.stride * info.height, info.stride * info.height + (info.stride / 2) * (info.height / 2) };
